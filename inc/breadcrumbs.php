...
function the_breadcrumb() {
	...
	global $post;
	$collegeHomeLink    = esc_url('https://citadel.edu/');
	$networkHomeLink    = network_home_url();
	$networkName        = get_blog_details(1)->blogname;
	$homeLink           = home_url();

	if ($showOnHome == 1) {
		echo '<ol id="breadcrumbs" itemscope itemtype="https://schema.org/BreadcrumbList"><li itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a href="' . $collegeHomeLink . '" itemprop="url"><span itemprop="title">The Citadel</span></a></li> ' . $delimiter;
		if ('The Citadel' == $networkName) {
			echo ' <li itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a href="' . $homeLink . '" itemprop="url"><span itemprop="title">' . $home . '</span></a></li>';
			if ( !is_home() && !is_front_page() ) {
				echo ' ' . $delimiter . ' ';
			}
		} else {
			echo ' <li itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a href="' . $networkHomeLink . '" itemprop="url"><span itemprop="title">' . $networkName . '</span></a></li>';
			if ( is_main_site() ) {
				if ( !is_home() && !is_front_page() ) {
					echo ' ' . $delimiter . ' ';
				}
				...
				if ( is_home() || is_front_page() ) {
					...
					echo ' <li itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem"><a href="' . $homeLink . '" itemprop="url"><span itemprop="title">' . $home . '</span></a></li> ' . $delimiter . ' ';
				}
			}
		}
		if (is_category()) {
